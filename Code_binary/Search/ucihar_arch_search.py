...
from models import UCIHARRunConfig
...
parser.add_argument('--path', type=str, default='./logs/ucihar/warmp20_gr')
...
parser.add_argument('--lr_schedule_type', type=str, default='cosine', choices=[None, 'cosine'])
# lr_schedule_param

parser.add_argument('--dataset', type=str, default='ucihar', choices=['ucihar'])
...
parser.add_argument('--weight_decay', type=float, default=0.) # 3e-4
...
""" net config """
parser.add_argument('--width_stages', type=str, default='64,64,64')
parser.add_argument('--n_cell_stages', type=str, default='1,1,1')
parser.add_argument('--stride_stages', type=str, default='1,1,1')
...
parser.add_argument('--arch_lr', type=float, default=5e-2)
parser.add_argument('--arch_adam_beta1', type=float, default=0.5)  # arch_opt_param
...
parser.add_argument('--target_hardware', type=str, default='s9', choices=['a31', 's9', 's10', 'cpu', 'gpu8', 'flops', None])
...
parser.add_argument('--grad_binary_mode', type=str, default='full', choices=['full_v2', 'full', 'two'])
...
if __name__ == '__main__':
    ...
    super_net = SuperProxylessNASNets(
        width_stages=args.width_stages, n_cell_stages=args.n_cell_stages, stride_stages=args.stride_stages, conv_candidates=args.conv_candidates,
        input_channel=run_config.data_provider.data_shape[0], seg_size=run_config.data_provider.data_length, n_classes=run_config.data_provider.n_classes, 
        width_mult=args.width_mult, bn_param=(args.bn_momentum, args.bn_eps), dropout_rate=args.dropout
    )
    ...
    arch_search_run_manager = ArchSearchRunManager(args.path, super_net, run_config, arch_search_config, args.target_hardware)
