...
class UniMiBDataProvider(DataProvider):

    def __init__(self, save_path=None, train_batch_size=256, test_batch_size=256, valid_size=5000,
                 n_worker=0):

        self._save_path = save_path
        train_dataset, test_dataset = load_dataset('dataset/unimib/')
    ...
    @property
    def data_shape(self):
        return 3, self.data_length  # C, length
    ...
    def save_path(self):
        if self._save_path is None:
            self._save_path = './dataset/unimib'
        return self._save_path

    @property
    def data_url(self):
        raise ValueError('unable to download UniMiB')
