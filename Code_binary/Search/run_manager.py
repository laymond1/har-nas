    ...
    def data_provider(self):
        if self._data_provider is None:
            if self.dataset == 'ucihar':
                ...
                self._data_provider = UCIHARDataProvider(**self.data_config)
            elif self.dataset == 'unimib':
                from data_providers.unimib import UniMiBDataProvider
                self._data_provider = UniMiBDataProvider(**self.data_config)
            elif self.dataset == 'wisdm2019':
                from data_providers.wisdm2019 import WISDM2019DataProvider
                self._data_provider = WISDM2019DataProvider(**self.data_config)
            elif self.dataset == 'wisdm':
                from data_providers.wisdm import WISDMDataProvider
                self._data_provider = WISDMDataProvider(**self.data_config)
            else:
...
class RunManager:
    ...
    def __init__(self, path, net, run_config: RunConfig, out_log=True, measure_latency=None):
        ...
        self.latency_estimator = LatencyEstimator(self.run_config.dataset, self.run_config.target_hardware)
    ...
    def net_latency(self, l_type='gpu4', fast=True, given_net=None):
        ...
        if l_type in ['a31', 's9', 's10']:
            predicted_latency = 0
            try:
                assert isinstance(net, ProxylessNASNets)
                # classifier 1 & 2
                predicted_latency += self.latency_estimator.predict(
                'Logit', net.classifier1.in_features, net.classifier1.out_features  # self.seg_length * 64
                )
                predicted_latency += self.latency_estimator.predict(
                'Logit', net.classifier2.in_features, net.classifier2.out_features  # 6
                )

                # blocks
                fsize = net.seg_size 
                for block in net.blocks: # only one operator in block (MixedEdge)
                    b_conv = block.conv
                    shortcut = block.shortcut
                    if isinstance(b_conv, MixedEdge):
                        b_conv = b_conv.active_op
                    if isinstance(shortcut, MixedEdge):
                        shortcut = shortcut.active_op
                    # 
                    if b_conv.is_zero_layer():
                        block_latency = self.latency_estimator.predict(
                           b_conv.module_str, [fsize, b_conv.in_channels], [fsize, b_conv.in_channels] # Zerolayer has same channels
                        )
                        # continue
                    else:
                        block_latency = self.latency_estimator.predict(
                            b_conv.module_str[4:], [fsize, b_conv.in_channels], [fsize, b_conv.out_channels],
                            kernel=b_conv.kernel_size, stride=b_conv.stride, dilation=b_conv.dilation 
                        )
                    predicted_latency += block_latency
                    fsize = math.floor((fsize + 2)//3) # 
            except Exception:
    ...
    def print_net_info(self, measure_latency):
    ...
    def train_one_epoch(self, adjust_lr_func, train_log_func):
        ...
        for i, (batch, labels) in enumerate(self.run_config.train_loader):
    ...
    def train(self, print_top5=False):
        ...
        for epoch in range(self.start_epoch, self.run_config.n_epochs):
            ...
            if (epoch + 1) % self.run_config.validation_frequency == 0:
                val_loss, val_acc = self.validate(is_test=True, return_top5=False)
                ...
                if print_top5:
                    val_log += '\ttop-5 acc {0:.3f}\tTrain top-1 {top1.avg:.3f}\ttop-5 {top5.avg:.3f}'.\
                        format(None, top1=train_top1, top5=train_top5)
                else:
