...
class GradientArchSearchConfig(ArchSearchConfig):
    ...
    def add_regularization_loss(self, ce_loss, expected_value):
        ...
        if self.reg_loss_type == 'mul#log':
            ...
            return ce_loss + reg_loss, reg_loss
...
class ArchSearchRunManager:
    ...
    def __init__(self, path, super_net, run_config: RunConfig, arch_search_config: ArchSearchConfig, target_hardware):
        # init weight parameters & build weight_optimizer
        self.run_manager = RunManager(path, super_net, run_config, True, measure_latency=target_hardware)
    ...
    def validate(self):
        ...
        valid_res = self.run_manager.validate(is_test=True, use_train_mode=False, return_top5=True)
    ...
    def warm_up(self, warmup_epochs=25):
        ...
        for epoch in range(self.warmup_epoch, warmup_epochs):
            ...
            for i, (batch, labels) in enumerate(data_loader):
                ...
                batch, labels = batch.to(self.run_manager.device), labels.to(self.run_manager.device)
                # compute output
                self.net.reset_binary_gates()  # random sample binary gates
                self.net.unused_modules_off()  # remove unused module for speedup
                output = self.run_manager.net(batch)  # forward (DataParallel)
                # loss
                ...
                losses.update(loss, batch.size(0))
        ...
        for epoch in range(self.run_manager.start_epoch, self.run_manager.run_config.n_epochs):
            ...
            for i, (batch, labels) in enumerate(data_loader):
                ...
                if epoch > 0:
                    ...
                    for j in range(update_schedule.get(i, 0)): # get(i, 0) if dict does not have i return 0
                        ...
                        if isinstance(self.arch_search_config, RLArchSearchConfig):
                            ...
                            self.write_log(log_str, prefix='rl', should_print=False) # rl.log
                        elif isinstance(self.arch_search_config, GradientArchSearchConfig):
                            arch_loss, ce_loss, reg_loss, exp_value = self.gradient_step() # arch_loss : ce_loss + reg_loss
                            used_time = time.time() - start_time
                            log_str = 'Architecture [%d-%d]\t Time %.4f\t Loss %.4f\t CELoss %.4f\t  RegLoss %.4f\t %s %s' % \
                                      (epoch + 1, i, used_time, arch_loss, ce_loss, reg_loss,
                                       self.arch_search_config.target_hardware, exp_value)
                            self.write_log(log_str, prefix='gradient', should_print=False) # gradient.log
                        else:
            ...
            for idx, block in enumerate(self.net.blocks):
                self.write_log('%d. %s' % (idx, block.module_str), prefix='arch') # arch.log
            self.write_log('-' * 60, prefix='arch')

            # validate
            if (epoch + 1) % self.run_manager.run_config.validation_frequency == 0:
                (val_loss, val_top1, val_top5), flops, latency = self.validate() # test print
    ...
    def rl_update_step(self, fast=True):
        ...
        batch, labels = self.run_manager.run_config.valid_next_batch
        batch, labels = batch.to(self.run_manager.device), labels.to(self.run_manager.device)
        ...
        for i in range(self.arch_search_config.batch_size):
            ...
            with torch.no_grad():
                output = self.run_manager.net(batch)
                acc1, acc5 = accuracy(output, labels, topk=(1, 5))
    ...
    def gradient_step(self):
        ...
        if self.arch_search_config.target_hardware is None:
            expected_value = None
        elif self.arch_search_config.target_hardware in ['a31', 's9', 's10']: # target_hardware
            ...
            raise NotImplementedError
        loss, reg_loss = self.arch_search_config.add_regularization_loss(ce_loss, expected_value)
        # compute gradient and do SGD step
        self.run_manager.net.zero_grad()  # zero grads of weight_param, arch_param & binary_param
        loss.backward()
        ...
        return loss.data.item(), ce_loss.data.item(), reg_loss.data.item(), expected_value.item() if expected_value is not None else None
