...
def build_candidate_ops(candidate_ops, in_channels, out_channels, stride, ops_order):
    ...
    name2ops = {
        ...
        'Zero': lambda in_C, out_C, S: ZeroLayer(in_channels=in_C, stride=S),
    }
    # add MBConv layers
    name2ops.update({
        '3x1_Conv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 3, S, 1),
        '5x1_Conv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 5, S, 1),
        '7x1_Conv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 7, S, 1),
        '9x1_Conv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 9, S, 1),
        #######################################################################################
        '3x1_DilatedConv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 3, S, 2),
        '5x1_DilatedConv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 5, S, 2),
        '7x1_DilatedConv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 7, S, 2),
        '9x1_DilatedConv': lambda in_C, out_C, S: ConvLayer(in_C, out_C, 9, S, 2),
        #######################################################################################
        '3x1_DepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 3, S, 1),
        '5x1_DepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 5, S, 1),
        '7x1_DepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 7, S, 1),
        '9x1_DepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 9, S, 1),       
        #######################################################################################
        '3x1_DilatedDepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 3, S, 2),
        '5x1_DilatedDepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 5, S, 2),
        '7x1_DilatedDepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 7, S, 2),
        '9x1_DilatedDepthConv': lambda in_C, out_C, S: DepthConvLayer(in_C, out_C, 9, S, 2), 
        #######################################################################################
        '3x5_DilatedMixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 2, S, 1),
        '3x5x7_DilatedMixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 3, S, 1),
        '3x5x7x9_DilatedMixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 4, S, 1),
        '3x5x7x9x11_DilatedMixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 5, S, 1),    
        #######################################################################################
        '3x5_MixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 2, S, 1),
        '3x5x7_MixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 3, S, 1),
        '3x5x7x9_MixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 4, S, 1),
        '3x5x7x9x11_MixedConv': lambda in_C, out_C, S: MixedConvLayer(in_C, out_C, 5, S, 1),        
        #######################################################################################
        #######################################################################################
...
op = MixedEdge(build_candidate_ops(['Zero'], 6, 64, 1, None))
op.probs_over_ops.data
...
class MixedEdge(MyModule):
    ...
    def __init__(self, candidate_ops):
        ...
        self.AP_path_wb = Parameter(torch.Tensor(self.n_choices))  # binary gates : not used
...
## ----- not used
    ...
    def forward(self, x):
        if MixedEdge.MODE == 'full' or MixedEdge.MODE == 'two':
            ...
            return sum(prob * op(x) for prob, op in zip(self.probs_over_ops, self.candidate_ops))
        else:
    ...
    def binarize(self):
        ...
        probs = self.probs_over_ops # softmax(arch params)
